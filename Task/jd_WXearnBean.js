...
if ($.isNode()) {
  if (process.env.JD_COOKIE && process.env.JD_COOKIE.indexOf('#') > -1) {
    jdcookie = process.env.JD_COOKIE.split('#');
  }else if(process.env.JD_COOKIE && process.env.JD_COOKIE.indexOf('#') > -1) {
    jdcookie = process.env.JD_COOKIE.split('\n');
  }else{
    jdcookie = [process.env.JD_COOKIE]
  }

  Object.keys(jdcookie).forEach((item) => {
    if (jdcookie[item]) {
      CookieArr.push(jdcookie[item])
    }
  })
}
