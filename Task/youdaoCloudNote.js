 ...
 * @Last Modified time: 2021-03-10 09:21:16
 */
   ...
   $.post(BodytaskUrl(`user?method=checkin`,body),async(error, response, data) =>{
    try{
      if (error) {
        ...
        // 反馈信息
        // console.log(`签到日期:`+ new Date(result.time).getFullYear()+'年'+new Date(result.time).getMonth()+'月'+new Date(result.time).getDay()+'日')
        console.log(`签到:`+result.success===1?` ✅ 成功`:` ❌ 重复签到`)
        console.log(`当前增加容量：`+result.space/1024/1024+`MB`)
      }}catch(e) {
          console.log(e)
        } finally {
        resolve();
 ...
 function BodytaskUrl(activity, body={}) {
  return {
    ...
    headers: {
      ...
      'Cookie': cookie,
      'User-Agent': 'you dao yun bi ji/7.0.5 (iPhone; iOS 14.3; Scale/3.00)',
    }
  }
}
